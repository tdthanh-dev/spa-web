// Home page data constants
export const faqData = [
  {
    question: "Xăm chân mày có đau không?",
    answer: "Tại Spa Dr Oha, chúng tôi sử dụng công nghệ xăm không đau với thuốc tê an toàn. Khách hàng chỉ cảm thấy hơi tê nhẹ, hoàn toàn không đau đớn. Công nghệ hiện đại chỉ tác động lên lớp thượng bì nên cảm giác rất nhẹ, đa phần khách hàng mô tả là thoải mái."
  },
  {
    question: "Xăm môi giữ màu được bao lâu?",
    answer: "Với công nghệ xăm môi hiện đại, màu sắc sẽ giữ được từ 3-5 năm. Spa Dr Oha cam kết bảo hành màu và hỗ trợ tái tạo khi cần thiết. Thực tế, mực hữu cơ bền 2-3 năm, mực vô cơ có thể giữ màu đến 5-7 năm nếu chăm sóc đúng cách."
  },
  {
    question: "Sau bao lâu có thể trang điểm?",
    answer: "Sau khi xăm chân mày và xăm môi, bạn cần chờ 7-10 ngày để vết thương lành hoàn toàn. Sau đó có thể trang điểm bình thường. Một số chuyên gia khuyên đợi thêm 1-2 tháng để màu mực ổn định nhất."
  },
  {
    question: "Có cần kiêng gì sau khi xăm không?",
    answer: "Sau khi xăm, bạn cần kiêng rượu bia, hải sản trong 3-5 ngày đầu. Ngoài ra nên tránh rau muống, thịt gà, đồ nếp và thực phẩm cay nóng trong 1-3 tuần. Không để nước bẩn, mỹ phẩm hay ánh nắng trực tiếp tác động vào vùng xăm, và tuyệt đối không gãi hoặc cạy vảy."
  },
  {
    question: "Xăm chân mày có tự nhiên không?",
    answer: "Công nghệ Hairstroke tại Spa Dr Oha tạo ra từng sợi lông mày siêu mảnh, tự nhiên như lông mày thật. Kỹ thuật phun/xăm hiện đại cũng giúp dáng mày mềm mại, hài hòa khuôn mặt."
  },
  {
    question: "Chi phí xăm chân mày và xăm môi là bao nhiêu?",
    answer: "Giá dịch vụ tại Spa Dr Oha dao động từ 2.500.000đ - 4.500.000đ tùy theo kỹ thuật và gói dịch vụ. Ngoài ra, còn có nhiều gói ưu đãi và combo tiết kiệm cho khách hàng."
  },
  {
    question: "Có cần tư vấn trước khi xăm không?",
    answer: "Có, việc tư vấn trước rất quan trọng. Chuyên gia sẽ phân tích khuôn mặt, tư vấn dáng mày phù hợp và chọn màu môi tự nhiên nhất. Đây là bước đảm bảo khách hàng hài lòng và kết quả bền đẹp."
  },
  {
    question: "Xăm môi có ảnh hưởng đến sức khỏe không?",
    answer: "Spa Dr Oha sử dụng mực xăm chuyên dụng, an toàn cho sức khỏe, đã được kiểm định. Quy trình vô trùng nghiêm ngặt đảm bảo an toàn tuyệt đối cho khách hàng, không gây kích ứng hay biến chứng."
  },
  {
    question: "Sau khi xăm có cần chăm sóc đặc biệt không?",
    answer: "Có, bạn cần bôi kem dưỡng theo hướng dẫn, tránh ánh nắng trực tiếp, không gãi hay cọ xát vùng xăm. Spa Dr Oha sẽ hướng dẫn chi tiết cách chăm sóc sau xăm. Thông thường, nên để vảy bong tự nhiên và giữ da luôn sạch sẽ, khô thoáng."
  },
  {
    question: "Có thể xóa xăm nếu không hài lòng không?",
    answer: "Trong trường hợp hiếm hoi không hài lòng, Spa Dr Oha hỗ trợ điều chỉnh hoặc tái tạo miễn phí. Ngoài ra, công nghệ xóa xăm bằng laser hiện đại cũng có thể loại bỏ hoặc chỉnh sửa màu mực an toàn."
  }
  
]

export const testimonials = [
  {
    name: "Nguyễn Minh Anh",
    service: "Xăm chân mày Hairstroke",
    review: "Chân mày xăm ở Spa Dr Oha rất tự nhiên, không ai biết tôi có xăm. Thật sự hài lòng với kết quả!",
    before: "/images/p1.jpg",
    after: "/images/p2.jpg"
  },
  {
    name: "Trần Thu Hà",
    service: "Xăm môi collagen",
    review: "Môi xăm ở đây rất đẹp, màu hồng tự nhiên. Không còn phải thoa son mỗi ngày, tiết kiệm thời gian trang điểm.",
    before: "/images/p3.jpg",
    after: "/images/p4.jpg"
    
  },
  {
    name: "Lê Thị Mai",
    service: "Combo mày + môi",
    review: "Dịch vụ chuyên nghiệp, không gian sạch sẽ. Tôi sẽ giới thiệu bạn bè đến Spa Dr Oha để làm đẹp.",
    before: "/images/p5.jpg",
    after: "/images/p6.jpg"
  },
  {
    name: "Phạm Thị Lan",
    service: "Xăm môi lip blushing",
    review: "Kỹ thuật lip blushing ở Dr Oha tuyệt vời! Môi tôi giờ có màu hồng tự nhiên rất đẹp, bền màu lâu.",
    before: "/images/p7.jpg",
    after: "/images/p8.jpg"
  }
]

export const services = [
  {
    id: 1,
    image: "/images/p1.jpg",
    alt: "Xăm chân mày Hairstroke tự nhiên",
    title: "Xăm chân mày Hairstroke",
    description: "Công nghệ xăm chân mày siêu mảnh, tạo sợi lông tự nhiên 99% giống thật, phù hợp phong thủy"
  },
  {
    id: 2,
    image: "/images/p2.jpg",
    alt: "Điêu khắc chân mày tán bột tự nhiên",
    title: "Điêu khắc chân mày tán bột",
    description: "Kỹ thuật xăm chân mày tán bột mềm mại, tạo dáng tự nhiên, phù hợp mọi khuôn mặt"
  },
  {
    id: 3,
    image: "/images/p3.jpg",
    alt: "Xăm môi collagen lip blushing",
    title: "Xăm môi collagen",
    description: "Xăm môi hồng tươi tự nhiên, giữ màu bền 3-5 năm, không cần thoa son mỗi ngày"
  },
  {
    id: 4,
    image: "/images/p4.jpg",
    alt: "Combo mày + môi ưu đãi",
    title: "Combo mày + môi",
    description: "Gói dịch vụ trọn bộ với giá ưu đãi, tiết kiệm chi phí, kết quả hoàn hảo"
  }
]

export const reasons = [
  {
    icon: "check",
    title: "Kinh nghiệm 10+ năm",
    description: "Đội ngũ chuyên gia giàu kinh nghiệm trong lĩnh vực xăm chân mày và xăm môi"
  },
  {
    icon: "star",
    title: "Công nghệ an toàn",
    description: "Mực nhập khẩu cao cấp, thiết bị hiện đại, quy trình vô khuẩn"
  },
  {
    icon: "award",
    title: "Chứng chỉ quốc tế",
    description: "Đội ngũ chuyên gia được đào tạo và cấp chứng chỉ quốc tế"
  },
  {
    icon: "heart",
    title: "Hậu mãi tận tâm",
    description: "Bảo hành màu trọn đời, hỗ trợ chăm sóc sau xăm 24/7"
  }
]

export const processSteps = [
  {
    number: 1,
    icon: "💬",
    title: "Tư vấn phong thủy",
    description: "Tư vấn dáng mày và màu môi hợp phong thủy, phù hợp với khuôn mặt"
  },
  {
    number: 2,
    icon: "✏️",
    title: "Vẽ demo & thử màu",
    description: "Vẽ demo dáng chân mày và thử màu môi để khách hàng hài lòng"
  },
  {
    number: 3,
    icon: "🎨",
    title: "Thực hiện xăm",
    description: "Thực hiện xăm bằng công nghệ mới nhất, không đau, không sưng"
  },
  {
    number: 4,
    icon: "📋",
    title: "Hướng dẫn chăm sóc",
    description: "Hướng dẫn chi tiết cách chăm sóc sau xăm để màu đẹp bền lâu"
  }
]
